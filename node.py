...
from networking import LoopbackFilter as LoopbackFilter

'''
Nodes connect to each other over links.  The node has a runloop that pulls
packets off the link's incoming packet Queue, runs them through its list of
filters, then places it in the nodes incoming packet queue for that interface
node.inq.

The Node's Program is has a seperate runloop in a different thread that is
constantly calling node.inq.get(). The program does something with the packet
(like print it to the screen, or reply with "ACK"), and sends any outgoing
responses by calling the Node's send() method directly.  The Node runs the
packet through it's outgoing packet filters in order, then if it wasn't
dropped, calls the network interface's .send() method to push it over the
network.
...
class Node(threading.Thread):
    ...
    """
    def __init__(self, interfaces, name, mac_addr=None, Filters=(), Program=None):
        ...
        # initialize the filters that shape incoming and outgoing traffic before it hits the program
        self.filters = [LoopbackFilter()] + [F() for F in Filters]

        # init the program that will be processing incoming packets
    ...
    def run(self):
        ...
        while self.keep_listening:
            for interface in self.interfaces:
                packet = interface.recv(self.mac_addr)
                if packet:
